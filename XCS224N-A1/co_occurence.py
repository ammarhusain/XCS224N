...
def computeCoOccurrenceMatrix(corpus, window_size=4):
    ...
    co_occurence_word_pairs = [[doc[word_idx], doc[word_idx + window_idx]] for doc in corpus for word_idx in range(len(doc)) for window_idx in range(-window_size, window_size+1) if window_idx != 0 and window_idx + word_idx >= 0 and window_idx + word_idx < len(doc)]

    for w1, w2 in co_occurence_word_pairs:
      M[word2Ind[w1]][word2Ind[w2]] += 1
