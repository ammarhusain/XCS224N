...
def naiveSoftmaxLossAndGradient(
    ...
    U_dot_vc = np.dot(outsideVectors, np.transpose(centerWordVec))
    softmax_out = softmax(U_dot_vc)
    P_o_given_c = softmax_out[outsideWordIdx]
    loss = -np.log(P_o_given_c)
    gradCenterVec =  -outsideVectors[outsideWordIdx] + \
    np.sum(np.array([softmax_out[i]*outsideVectors[i] for i in range(outsideVectors.shape[0])]),0)
    gradOutsideVecs = np.array([y_w*centerWordVec for y_w in softmax_out])
    # Implement gradient when w = o
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outWord in outsideWords:
      outWordIdx = word2Ind[outWord]
      lossCurr, gradCentCurr, gradOutCurr = word2vecLossAndGradient(centerWordVec, outWordIdx, outsideVectors, dataset)
      loss += lossCurr
      gradCenterVecs[centerWordIdx,:] += gradCentCurr
      gradOutsideVectors += gradOutCurr
