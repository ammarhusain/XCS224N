...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        last_hidden_unstack = torch.cat((last_hidden[0,:], last_hidden[1,:]), dim=1)

        init_decoder_hidden = self.h_projection(last_hidden_unstack)
        last_cell_unstack = torch.cat((last_cell[0,:], last_cell[1,:]), dim=1)
        init_decoder_cell = self.c_projection(last_cell_unstack)
