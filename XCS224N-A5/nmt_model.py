...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        # source_padded_chars shape: (max_sentence_length, batch_size, max_word_length)
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device)
        #print("source_padded_chars_s ", source_padded_chars.shape)
        # target_padded_chars shape: (max_sentence_length, batch_size, max_word_length)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device)
        #print("target_padded_chars_s ", target_padded_chars.shape)

        #print(target_padded_chars)

        target_padded = self.vocab.tgt.to_input_tensor(target, device=self.device) # shape: (max_sentence_length, batch_size)
        #print("target_padded_s ", target_padded.shape)
        #print(target_padded)

        # Encode using the character tensors
        ...
        if self.charDecoder is not None:
            ...
            #AH: OG target_chars = target_padded_chars[1:].view(-1, max_word_len)
