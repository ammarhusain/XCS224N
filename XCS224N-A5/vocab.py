...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for s in sents:
          s_id = []
          for w in s:
            w_id = [self.start_of_word]
            for c in w:
              w_id.append(self.char2id[c])
            w_id.append(self.end_of_word)
            s_id.append(w_id)
          word_ids.append(s_id)
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        char_idcs = self.words2charindices(sents)
        padded_char_idcs = pad_sents_char(char_idcs, self.char2id['<pad>'])
        padded_char_idcs_tensor = torch.tensor(padded_char_idcs, device=device)
        # Reshape tensor to (max_sentence_len, batch_size, max_word_length)
        padded_char_idcs_tensor = padded_char_idcs_tensor.transpose(0,1)

        return padded_char_idcs_tensor
        ### END YOUR CODE
